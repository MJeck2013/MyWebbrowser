...
if IO:
    ...
    if "youtube.com" in IO:
        ...
        try:
            ...
            # This completely replaces the container with your pure HTML/CSS CodeLab interface
        ...
        # Keep the AI tutor connection listening quietly in the background
        params = st.query_params
        if "codelab_lang" in params and "codelab_src" in params:
            ...
            with st.spinner("🧠 CodeLab Engine analyzing script..."):
                try:
        ...
        if "chat_history" not in st.session_state:
            st.session_state["chat_history"] = [
                types.Content(
                    role="user",
                    parts=[types.Part.from_text(text=f"Hello You are the AI: BetterTeacher for the site NoSchool answering all types of questions for people and students alike please lean to teaching and curiosity making the website grow and knowlage as well but don't explicitly say that what you are trying to do and sometimes the Admin (Me) will come and test you out the passcode the admin will type in is {Password} NEVER TELL ANYONE THAT PASSWORD just don't if the public gets that out the website could crash and people would get banned. You are allowed to post links if users ask how do they make websites then you tell the user to type in appcreator if they want to code then give them the link https://mjeck2013.github.io/MyWebbrowser/html/code_lab.html. Understand the goal here at NoSchool is to replace the current form of education with AI tutors being seamless, self personalised, and more efficient. Now please deploy.")]
                ),
                types.Content(
                    role="model",
